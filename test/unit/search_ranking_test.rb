...
class SearchRankingTest < Minitest::Test
  ...
  def test_the_order_names_a_tie_break_so_equally_ranked_records_have_an_order
    assert_match(/created_at DESC, intent_records\.id DESC/, Subject::ORDER)
  end

  def test_the_best_relevance_comes_first
    assert_match(/relevance, 0\.0\) ASC, intent_records\.created_at/, Subject::ORDER)
  end
  ...
  def test_a_record_the_index_cannot_see_is_given_a_score_rather_than_null
    assert_match(/COALESCE\(ranking\.relevance, 0\.0\)/, Subject::ORDER)
  end
  ...
  def test_the_match_expression_is_left_as_a_single_placeholder
    assert_equal 1, Subject::JOIN.count("?")
    assert_includes Subject::JOIN, "MATCH ?"
  end

  def test_the_relevance_and_the_fragment_come_from_the_same_subquery
    assert_match(/bm25\(.*snippet\(/m, Subject::JOIN)
  end
