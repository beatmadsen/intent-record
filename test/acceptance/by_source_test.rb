...
class BySourceTest < Minitest::Test
  ...
  OTHER_JIRA = { "system" => "jira", "uri" => "https://acme.atlassian.net/browse/ACME-43" }.freeze
  COMMIT = "1111111111111111111111111111111111111111".freeze

  def test_lists_only_the_intents_recorded_against_the_given_uri
    wanted = record_intent!(stakeholder_references: [JIRA])
    record_intent!(stakeholder_references: [OTHER_JIRA])

    json = run_cli_ok!("by-source", URI)

    assert_equal([wanted["intent_id"]], json["intents"].map { |i| i["intent_id"] })
  end

  def test_each_listed_intent_carries_its_own_commits
    record_intent!(commits: [COMMIT], stakeholder_references: [JIRA])

    json = run_cli_ok!("by-source", URI)

    assert_equal COMMIT, json["intents"].sole["asset_versions"].sole["external_id"]
  end

  def test_lists_the_sources_the_uri_matched
  ...
  def test_substring_match_finds_ticket_key_across_systems
    a = record_intent!(stakeholder_references: [JIRA])
